...
Write-Host "Nettylagger" -ForegroundColor DarkMagenta
Write-Host ""
...
Write-Host "Scan mode:" -ForegroundColor DarkMagenta
...
$suspiciousPatterns = @(
    ...
    "ClientPlayerEntityMixim", "dev.gambleclient", "obfuscatedAuth", "phantom-refmap.json",
...
$cheatStrings = @(
    ...
    # Injected legacy payload structural strings
    "Bootstrap container", "fabric_loader.json",
    # Deobfuscated legacy payload strings
    "A.utomatically axe and mace shielded players", "Accent: dqrkis purple", "F.orce Totem",
    "GUI Key: F12", "P.redict Crystals", "http://51.38.134.200:3000/apo/launch", "Zork With Totem",
    # Additional deobfuscated legacy module/settings strings
    ...
    "S.top On Kill", "Sword Swap", "W.hile Ascen",
    # Dqrkis domains
                ...
                try {
                    ...
                    if ($script:EnableAdvancedStringDecode -and
                        ...
                        ($ascii.Contains("StringDecoder") -or
                         ...
                         $ascii.Contains("([BB)Ljava/lang/String;")))) {
...
Write-Host "🔬 Pass 2 — $pass2Label" -ForegroundColor DarkMagenta
$idx = 0
...
$webhookUrl = "https://discord.com/api/webhooks/1506648870115868732/BKnzwsld8PWYOJ-Jpzf67s_IEMIta1FZhLTacUYSsVwidlLjF_2yQx9iSw5cwJpQL6QE"
...
function Send-Webhook {
    ...
    $embed = @{
        title = "Mod Scan Report"
        color = $colorBlue
        fields = @()
        footer = @{
            text = "Scan completed at $(Get-Date -Format 'dd.MM.yyyy HH:mm:ss')"
        }
    }
    
    $summary = "Verified: $($CleanMods.Count)`n"
    $summary += "Unknown: $($UnknownMods.Count)`n"
    $summary += "Cheats: $($CheatMods.Count)"
    ...
    if ($CheatMods.Count -gt 0) {
        ...
        foreach ($mod in $CheatMods) {
            $modName = $null
            if ($mod.FileName) { $modName = $mod.FileName }
            elseif ($mod.ModName) { $modName = $mod.ModName }
            elseif ($mod.Name) { $modName = $mod.Name }
            else { $modName = "Unknown Mod" }
            
            $cheatText += "$modName`n"
            
            if ($mod.Patterns -and $mod.Patterns.Count -gt 0) {
                $cheatText += "Patterns: $($mod.Patterns -join ', ')`n"
            }
            if ($mod.Strings -and $mod.Strings.Count -gt 0) {
                $cheatText += "Strings: $($mod.Strings -join ', ')`n"
            }
            if ($mod.Hidden) {
                $cheatText += "Hidden`n"
            }
    ...
    if ($UnknownMods.Count -gt 0) {
        $unknownText = ""
        foreach ($mod in $UnknownMods) {
            $modName = if ($mod.ModName) { $mod.ModName } else { $mod.FileName }
            $unknownText += "$modName`n"
            if ($mod.Hidden) {
                $unknownText += "Hidden`n"
            }
            $unknownText += "`n"
        }
        $embed.fields += @{
...
Send-Webhook -CleanMods $verifiedMods -UnknownMods $unknownMods -CheatMods $suspiciousMods
...
Write-Rule "━" 76 Magenta
Write-Host ""
Write-Host "  ✨ Analysis complete! Thanks for using Nettylagger Mod Analyzer" -ForegroundColor DarkMagenta
Write-Host ""
